...
impl Evaluator for Fetch {
	async fn evaluate(
		...
		let artifact = if fetch.unpack {
			...
			let artifact = self.unpack(builder, temp_path, archive_format).await?;
			tracing::trace!(
			...
			builder.checkin(&temp_path).await?
		};
...
impl Fetch {
	async fn unpack(
		...
		let unpack_temp_path = builder.create_temp_path();
		...
		// Checkin the temp.
		let artifact = builder.checkin(&unpack_temp_path).await?;
