...
impl Value {
	...
	pub async fn with_path(tg: &Client, package_path: &Path) -> Result<Self> {
		...
		// let lockfile = Blob::with_reader(tg, lockfile.as_slice()).await?;
		// let lockfile = File::builder(lockfile).references(references).build();
		...
		// // Create the package.
		// let package = Self {
		// 	artifact: Ref::with_value(directory.into()),
