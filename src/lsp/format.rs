...
mod tests {
	...
	macro_rules! test {
		($before:expr, $after:expr$(,)?) => {{
			...
			let server = Server::new(path, None).await.unwrap();
			let lsp = lsp::Server::new(&server);

			// Test.
			let left = lsp.format(indoc::indoc!($before).to_owned()).await.unwrap();
	...
	async fn test_reindent_multi_line_nested() {
		...
		test!(
			r#"
				...
				export default tg.target(async ({ target }: Args) => {
					return std.run(
						t`
